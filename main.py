...
def choose_noble_deterministic(eligible: list[Noble]) -> Optional[Noble]:
    """
    Deterministyczny wybór arystokraty:
    - najpierw maksymalny prestiż,
    - tie-break: najmniejsze id.
    """
    if not eligible:
        return None
...
def run_game_nn():
    ...
    NnAgent nie wybiera arystokraty — po apply_move bierzemy eligible i wybieramy deterministycznie.
    """
    print("="*50)
    print(" Uruchamianie trybu gry: Splendor NN ".center(50))
    print("="*50)

    # --- Inicjalizacja NN agenta ---
    print(f"Wczytywanie mistrza NN z pliku '{NN_CHAMPION_FILE}'...")
    if not os.path.exists(NN_CHAMPION_FILE):
        print("\n[BŁĄD] Plik NN mistrza nie został znaleziony!")
        print("Proszę najpierw uruchomić trening za pomocą komendy:")
        print("  python train_nn.py --gens 100")
        sys.exit(1)
    ...
    try:
        ...
        ai_agent = NnAgent(
            theta=theta,
            hidden1=nn_cfg.HIDDEN1,
            hidden2=nn_cfg.HIDDEN2,
            act=nn_cfg.ACT,
            rng_seed=nn_cfg.SEED_BASE,
        )
    ...
    while running and not game.is_game_over():
        ...
        if current_player_agent is human_player:
                    ...
                    if move not in valid_moves:
                        ...
                        if isinstance(move, (TakeThreeGems, TakeTwoGems)):
                            ...
                            if cur_total + added > 10:
                                ...
                                msg = "Ruch niedozwolony: sprawdź dostępność żetonów i zasady."
                        raise ValueError(msg)
            ...
            # apply -> eligible -> deterministyczny wybór arystokraty -> finalize
